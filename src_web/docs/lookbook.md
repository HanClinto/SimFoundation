...
The X at the right of the portrait header means Deselect active pawn. It clears browser control selection, closes its contextual menu and hides its queue without cancelling any activity or changing draft status. A self-selected pawn highlight clears; an independently selected object/target remains inspectable. Deselection also works in Recorded view and during placement. With no active pawn, contextual menus retain Inspect/Select Person but omit personal verbs. Each action tile retains its own X for owner-aware cancellation; those controls never deselect the pawn. Follow remains an independent camera choice.

Each map has one selection-and-orders area containing portrait shortcuts, target details and the action tray. Clicking a portrait selects and highlights that person on the map, updates the inspected target and makes them the command recipient without drafting or moving them. When actor and target are the same, duplicate portrait/name/activity details are omitted; needs and inspection/Orders controls remain. When they differ, the active person's header stays visible and the target row is explicitly labelled Target. Another pawn's target row offers Control to switch recipients deliberately. Clicking an object or opening a dossier leaves the actor selected. Follow stays pinned independently until explicitly changed.

Target clicks use a Subject > Object > Verb hierarchy styled with the existing 98.css menu extension. The active person's portrait and name head the popup without a repeated breadcrumb. Visible overlapping targets and the underlying floor have separate icon-labelled branches. A sole target expands automatically; multiple targets wait for explicit selection. Expanding a branch never issues an order. Right arrow enters its verbs; Left returns to the target. Clicking elsewhere on the map dismisses the popup without opening another one on that click. Inspect and Select Person remain outside the command tree. A future optional Adjective/modifier level can qualify a verb when the simulation supports a real choice; no placeholder modifiers are shown.
...
Action tiles use a shared progress contract with action-specific measurements and an elapsed-time fallback. Active Eat, Sleep and Relax show a percentage bar and approximate remaining in-game minutes once activity at the furniture begins. Sleep estimates the remaining rest deficit to95 using net recovery0.30 per minute; Eat/Relax use their existing12/30-step durations. Go Here/Retreat, routine travel, job travel and field recovery travel show remaining A\* route tiles to their current destination, with no percentage or arrival-time claim. Meal collection counts the current pantry leg, then the seat leg. Door opening may take extra time; inaccessible destinations say No route, never zero tiles. Tactical approach retains elapsed time until its executor exposes an appropriate route measurement.
