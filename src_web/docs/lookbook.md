...
Each map has a compact portrait strip and its own active person. The active portrait and mint map marker identify who receives personal orders. The inspected target retains the existing selection treatment; actor, target and Follow are independent. Selecting a person never changes simulation control ownership. Clicking an object or opening a dossier leaves the actor selected.

Target clicks use a Subject > Object > Verb hierarchy styled with the existing 98.css menu extension. The active person's portrait and name head the popup; a pinned path includes the selected object and hovered/focused verb. Visible overlapping targets and the underlying floor have separate branches. Expanding a branch never issues an order. Right arrow enters its verbs; Left returns to the target. Inspect and Select Person remain outside the command tree and never issue personal work. A future optional Adjective/modifier level can qualify a verb when the simulation supports a real choice, such as a weapon or conversation topic; no placeholder modifiers are shown.
...
Commands default to Add to Queue without a placement preview; the separate submission selector offers Do Now for safe current-action replacement while retaining pending work. The queue holds at most eight intentions including the current one. Construction still uses Confirm/Cancel. Double-click continues to inspect, dragging pans, and menus stay within their map viewport. Recorded mode remains inspection-only. The strip shows the actual current action, recovery and block reasons, with Cancel Current Action. An Actions disclosure shows the current intention, pending entries with individual cancellation, Retry for failed starts, and Clear Pending. Cancellation preserves spent supplies and physical cargo ownership; field cargo is put down where its carrier stands. No pending action reserves cargo or spends supplies before it starts.
