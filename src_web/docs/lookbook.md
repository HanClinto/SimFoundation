...
Commands submit immediately without a placement preview; construction still uses Confirm/Cancel. Double-click continues to inspect, dragging pans, and menus stay within their map viewport. Recorded mode remains inspection-only. The strip shows the actual current action, action recovery and block reasons, with a compact Cancel Current Action control. Cancellation preserves spent supplies and physical cargo ownership; field cargo is put down where its carrier stands. This remains one replaceable action, not an unimplemented queue. Multi-action queues follow in M4.
