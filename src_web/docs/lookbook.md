...
Target clicks use the existing flat 98.css menu extension: Go Here on ground, Hold Position on ground/self, Stabilize on another person, Attack and Engage From Here on the adversary, and Recover to Extraction on field cargo. Attack includes physical approach to a firing position; Engage From Here holds the current position. Inspect stays separate; Select Person explicitly changes the actor. Unsupported personal-use actions are omitted. The menu presentation is provisional and can later change independently of the action descriptors. Disabled reasons update without replacing keyed controls.
