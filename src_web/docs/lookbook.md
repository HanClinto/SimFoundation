...
Active Eat, Sleep and Relax tiles show a compact progress bar and approximate remaining in-game minutes once the pawn is at the furniture and making progress. Automatic and player-directed routines use the same display. Sleep estimates the remaining rest deficit to95 using net recovery0.30 per minute (0.35 gained minus0.05 natural decay); its bar is elapsed sleeping steps divided by elapsed plus estimated remaining steps. Eat and Relax use their existing12/30-step durations. Tooltips specify uninterrupted activity and paused state. Travel, pending intentions, blocked/incapacitated routines and other action types have no fabricated progress or ETA. Tile dimensions stay fixed as the bar appears/disappears. Estimates are derived, not saved; schema43 is unchanged.
