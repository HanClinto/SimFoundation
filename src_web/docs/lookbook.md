...
Ground clicks offer Go Here and Inspect. Go Here submits immediately without a placement preview; construction still uses Confirm/Cancel. Double-click continues to inspect, dragging pans, and menus stay within their map viewport. Recorded mode remains inspection-only. The strip shows current movement and block reasons, not an unimplemented queue. M1 replaces one movement intention; multi-action queues and other contextual interactions follow later milestones.

Undrafted base staff temporarily leave autonomy for a valid direct move and resume it after arrival and recovery. Drafted staff continue holding. Cargo, appointments, casualties, field recovery and expedition phases keep their ownership protections. See [decision 004](decisions/004-direct-person-control.md) for input, ownership and future queue contracts.
