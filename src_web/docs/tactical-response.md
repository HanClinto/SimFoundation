...
Tactical Response provides Draft, Release, condition and threat reporting, Locate, and Control on Map. The inline Response link opens this inspector; personal movement, Hold, Attack, Engage From Here and Stabilize use the map's shared subject/target/action queue. The separate Encounter Sandbox facility shortcut contains test-encounter creation, not ordinary staff operations. This slice uses the existing pause and simulation speeds, not a separate combat clock. Normal speed is recommended while learning action cadence. One combat step is one existing simulation tick; the facility clock still advances a minute per tick. This is provisional tactical pacing, not a physical claim about weapon timing.
...
1. Select Caleb and Lena (optionally Priya), draft them, and use Control on Map or their portraits to issue Go Here to nearby positions around 68,55 inside containment. Personal commands follow Queue / Do Now without a placement confirmation. Travel and doors remain physical.
2. Open Encounter Sandbox from the facility inspector. Place 049-2 opens an explicit sandbox placement preview at the default 72,55; Confirm creates one already-provoked instance on a valid floor tile, while Cancel creates nothing. Nothing is spawned by opening the window or in an ordinary new site. Two or three drafted responders are required. Placement requires World view and cannot overwrite another active placement; switching to Recorded prevents confirmation.
3. Issue Engage From Here or Attack separately to responders who should cover the target. Hold does not attack automatically. Select a responder on the map to see their range and line of sight.
