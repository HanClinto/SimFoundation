...
The annex model, executor, controller commands, project art, placement exclusions, register and validation have been deleted. `construction` now holds only current material accounting (`MaterialStock` in `material-stock.ts`): available materials and a preferred stockpile position. Physical stacks and current surface/vessel orders are validated for conservation. This is not an annex compatibility adapter.
