...
`environment` owns exposure sources, surface orders, and facility maintenance policy. Sources specify position, radius, damage kind, and dose. Cardinal flood traversal includes blocking surfaces but does not pass through them. Newly opened space is reached on the next tick. Damage is batched into one map update, using each reached layer's resistance. AN-001 is scenario data for this mechanism, not a special mutation path. Its five-by-five enclosure uses normal ceramic surface records.
...
`observations.knownSurfaces` preserves the last recorded layers beside existing tile timestamps. Browser projections substitute these records for live surfaces. Material and Condition overlays explicitly select floors or structures. Engineering exposes recorded layers, replacement work, and door controls. Schema 24 validates materials, integrity, topology agreement, orders, source bounds, cargo identity, and shared-stock conservation. Earlier development saves are incompatible.

The AN-001 trial state machine, special barrier counters, dedicated allowance, scripted protocol UI, and its tests have been retired. The Library no longer grants research capabilities or fabricates assessment evidence. Scenario capabilities still gate clinical behavior. There is no general research progression, structural collapse, fire, fluid, or persistent contamination model in this slice; SCP-9620's existing authored job progression is retained.
