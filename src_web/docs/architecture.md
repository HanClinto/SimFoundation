...
Construction register controls reside in an independent modeless Construction window. It communicates with Camera Feed through plan and locate callbacks; Camera Feed no longer queries register or laboratory-selector DOM elements. This lets either window close or move independently while preserving simulation projects and camera placement behavior.
...
`containmentTrial` owns the original AN-001 directed-contact experiment, primary material/protocol, primary and secondary integrity, finite material allowance, delivery stage, maintenance policy, pending work, trial counters, per-barrier observed conditions, and bounded evidence. Primary failure replaces the wall at (70, 61) with floor. Continued contact degrades a composite secondary hatch at (70, 63); its failure also opens the tile. `closed-door` blocks both A\* traversal and observation rays, unlike ordinary open doors. A small catch enclosure and service access routes are authored into the starting map.

Barrier work progresses through material collection, same-carrier delivery, and engineering fitting at (70, 60). The kit restores both the primary panel and secondary lining; completed assembly waits if either footprint is occupied. Research prepares exposure. Chemical and impact resistance remain separate, with optional protective isolation. After observation, maintenance discovery can queue replacement at recorded primary integrity <=55 once exposure ends. Failed-wall packages use emergency priority 95; the job scheduler can preempt routine work without discarding progress, but preserves appointments, urgent-needs exclusions, and cargo-bound workers. Hidden damage does not trigger maintenance or refresh engineering records.

Save schema 23 includes enclosure conditions, policy, material stages, and recorded inspections. Save validation checks map/condition agreement and pending job destinations/skills. Earlier development saves are incompatible, not migrated. Damage remains localized to two structural tiles; there is no free-roaming specimen or spreading contamination model.
