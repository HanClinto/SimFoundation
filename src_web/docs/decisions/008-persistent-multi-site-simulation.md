...
Agreed terminology is Entity for persistent identity and Pawn for agency/action execution, not mobility. Staff is a role and anomaly is a classification. Carryability and mobility are independent capabilities. Carrying a disabled, injured or dead pawn retains its identity and associated records, with a carrier-relative location and once-only owner-driven ticking; it does not create a second proxy cargo entity. Death and medical-transport gameplay are not introduced merely by adopting this representation.

The target replaces a separate drafted/combat mode with an autonomy enabled/disabled policy. Disabling autonomy prevents new self-selected needs/schedule/job/behavior commitments; it does not change player permission, stop physiology, cancel active work, clear explicit queues or grant resources. Current commitments finish unless explicitly cancelled safely. Player and autonomous intents share the same executor, and ordinary movement never requires combat enrollment. See the plan's agreed control policy and E8/E9/T4 checklist for implementation gates; current runtime drafting has not yet been removed.
...
Saves are ordinary JSON snapshots of authoritative state with root-object/version checks and try/catch only, not a persistence framework or ownership validator. Current-version contents are trusted. Authored JSON site data, optionally using ASCII tile rows, supplies Site 828 and specialized integration fixtures through the same headless loading path. Restoring a campaign preserves identity/progress; instantiating a template allocates new identities and remaps its local references. No migrations or duplicate gameplay-validation layer.
