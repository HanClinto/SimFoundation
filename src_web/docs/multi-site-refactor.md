...
The user's latest decision supersedes the in-place migration strategy below: archive the existing simulation in `src/simulation_legacy`, retain it for reference, and build a clean replacement in `src/simulation`. Preserving transitional UI stability is not a requirement. See [decision 009](decisions/009-replacement-simulation-core.md) and the [new core README](../src/simulation/README.md).

The replacement now follows [decision 010](decisions/010-sequential-core-and-catalog.md): generic core mechanics, named catalog definitions, per-action classes, and stable sequential ticking against current state. The proposal/resolver layer has been removed. Plain entities/pawns, material-tag diets, optional needs/control/autonomy, carried pawn identity, authored JSON placements, tiny snapshots and prepared multi-site transfers are tested headlessly. The old application/test imports explicitly target the archive; the new core imports none of it. The ledger below records historical work and the remaining feature intentions, not completion status of the replacement. Port useful behavior selectively; do not preserve legacy state/API shapes to satisfy old tests.

Status: approved direction, implementation started 2026-09-09; tracked in [#24](https://github.com/HanClinto/SimFoundation/issues/24). M0 is complete. The headless persistent-site core and application command boundary now operate, with M1-M3 integration still in progress. The browser has not switched to campaign storage. This plan precedes new SCP scenarios and supersedes the temporary-site assumptions and feature ordering in [expedition priorities](scp-expedition-priorities.md), not its content recommendations.
...
The next checkpoint adds [sites.ts](../src/simulation_legacy/sites.ts): an ordinary serializable site collection, neutral site creation, guarded disposal, explicit site context/update, ownership checks and one sorted global tick over all retained sites. Local systems and the shared runner now accept site context without `version` or `expeditions`. Sites can have no SCP-999 and no startup stock, people, cameras or furniture. Newly generated split objects and vessels in these stores have site-qualified IDs that remain stable.

[simulation-controller.ts](../src/application/simulation-controller.ts) exposes typed site-scoped physical work and personal queue commands, detached snapshots, global pause and per-site incident transitions. [action-queue-core.ts](../src/simulation_legacy/action-queue-core.ts) is the single queue state machine used by both the existing campaign adapter and the new site host. Local movement and interaction execution are shared as well; no empty expedition state is fabricated. Site queues run around the same ordinary local work runner.
...
[site-transfers.ts](../src/simulation_legacy/site-transfers.ts) adds cross-site transfer ownership to the headless controller. Ordinary hauling and personal movement prepare cargo/people at a chosen loading tile. The dispatch command checks that preparation, then atomically removes the selected records from the origin into a transit payload. It supports raw-material quantities, packed objects, sealed vessels with contents and attached sources, and stable ready personnel carrying their personal equipment, responder supplies and schedule. It does not yet auto-schedule preparation as one transport order.

Transit payloads run needs and vessel wear once per global step. Arrival occurs after site work so newly arrived records do not receive another local tick. Blocked arrivals keep their payload and expose a reason. Return-to-origin takes a full new travel duration; repeat return and completed-transfer commands cannot duplicate delivery. Both endpoints block disposal while a transfer remains active. Completion history retains endpoint names, identities, cargo and personnel. New source IDs are site-qualified like split object and vessel IDs.

Tests cover actual hauling to loading, refusal before preparation, partial stack conservation, loaded-vessel/source identity, continued wear including breach while blocked, return-to-origin, replay, and an agent walking to departure, travelling with supplies and building at the destination. A separate A -> B -> C chain builds with delivered stock at B and forwards the remainder with stable identity to C. No endpoint has headquarters privileges.

This is partial M4 ahead of the browser cutover, providing a concrete test of the ownership design. Current restrictions: no injured/incapacitated personnel, active encounters, unresolved work/queues, carried work cargo, resident/threat capture, automatic loading plan, fuel or vehicle animation. No quest is required to ship freight. The existing browser's vessel transport and expedition lifecycle are not yet redirected to this mechanism. Do not mark M4 complete until that convergence and production persistence are done.
...
| ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| [state.ts](../src/simulation_legacy/state.ts)                                | Global clock, personnel and one site's local systems share GameState                          | Campaign state contains sites, explicit ownership, transfers and cross-site records                      |
| [tick.ts](../src/simulation_legacy/tick.ts)                                  | Advance the base, filter/reconcile away personnel, then run a different expedition tick       | One site runner applied to every site on one clock, plus a transit runner                                |
| [expeditions.ts](../src/simulation_legacy/expeditions.ts)                    | Field projection fabricates empty subsystems; return deletes its site                         | Sites retain their real systems independently of operations                                              |
| [expedition-site.ts](../src/simulation_legacy/expedition-site.ts)            | Scenario creates a fresh site per dispatch; recovery list controls eligibility and completion | Site setup creates initial conditions; operation and quest definitions reference existing sites/entities |
