...
The first replacement slice is executable and tested headlessly: plain entities/pawns, shared action execution, entity-owned tick proposals, deterministic snapshot resolution, optional needs/control/autonomy, carried pawn identity, authored JSON sites, tiny snapshots and prepared multi-site transfers. The old application/test imports explicitly target the archive; the new core imports none of it. The ledger below records historical work and the remaining feature intentions, not completion status of the replacement. Port useful behavior selectively; do not preserve legacy state/API shapes to satisfy old tests.
