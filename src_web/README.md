...
**Direct person control (issue #23, M1/M2):** select a person using the map's portrait strip, then click a target for its 98.css context menu. Ground offers **Go Here** and **Hold Position**, people offer **Stabilize** and explicit **Select Person**, the adversary offers **Engage From Here**, and field cargo offers **Recover to Extraction**. No separate orders window or movement confirmation is needed. Inspection does not change the active person. The strip shows current action and block reasons, with **Cancel Current Action** preserving cooldowns and spent supplies; cancelled field cargo is put down at the carrier's tile. Undrafted staff resume autonomy after movement or stabilization; explicit Hold/engagement retains drafting. Protected cargo, appointments and expedition commitments still prevent conflicting orders. This is one replaceable action, not a pending queue; engagement does not yet approach targets. Construction previews and double-click inspection are unchanged. Enter opens a selected tile's interactions; Shift+F10 opens the selected target's menu, or ground at map center when none is selected. **Save schema 39 requires a fresh development site.**
