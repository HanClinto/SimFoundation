...
Commands default to **Add to Queue**, with up to eight intentions per person including the current action. The map toolbar's **Orders** menu selects Add to Queue or **Do Now** for that window. Do Now safely replaces the current action and retains pending work. The horizontal action tray shows target icons with verb captions, X controls, **Retry** for failed starts and **Clear Pending**. Drag pending tiles to reorder; Alt+Left/Right and Alt+Home/End also move them. The current action stays pinned and can be cancelled, not dragged. Cancellation preserves cooldowns and spent supplies, then advances to pending work; cancelled field cargo is put down at the carrier's tile. Pending entries reserve nothing before execution. Undrafted staff resume autonomy after the queue drains; explicit Hold/Attack/engagement retains drafting. Protected appointments, cargo and mission phases retain ownership. Construction previews and double-click inspection are unchanged. Shift+F10 opens the selected target's menu; Right/Left enter and leave verb branches. Optional future verb modifiers are not yet domain choices. **Save schema 41 requires a fresh development site; this tray update does not change it.**
