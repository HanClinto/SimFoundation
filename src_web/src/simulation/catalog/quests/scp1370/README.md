...
Run `npm run sim -- --scenario scp1370` from `src_web`. Use `brief` and `status`, then `deploy field-agent alex` and `start`. Alex automatically arrives at entry and fills the handler role. Alex is labeled `@2` (the exhibit is `@1`); names and labels both work in commands, such as `order alex take exhibit`. `inspect station` lists plans, and `order alex study station safe-exhibit` performs the observation after the exhibit has been physically brought nearby. Damage/loss, handler incapacitation and deadline failure are separate from successful secure recovery. The colocated transcripts include deployment/start so team selection is part of the reproducible solution.
